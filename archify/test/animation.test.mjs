...
test('signal-flow preset reaches the page, SVG, and motion export surface', () => {
  ...
  assert.match(html, /<html lang="zh-CN" data-theme="dark" data-preset="signal-flow">/);
  assert.match(svgBlock(html), /data-preset="signal-flow"/);
  assert.match(html, /content: "信号流"/);
  ...
  assert.match(html, /aria-label="图表视图控制"/);
  ...
  assert.match(html, /当前浏览器无法导出 WebM/);
  assert.match(html, /WebM 动态导出需要轨迹动画和浏览器 MediaRecorder 支持/);
...
test('blueprint preset reaches every visual surface without changing the default', () => {
  ...
  assert.match(html, /<html lang="zh-CN" data-theme="dark" data-preset="blueprint">/);
  assert.match(svgBlock(html), /data-preset="blueprint"/);
  assert.match(html, /content: "蓝图版 \/ 修订 01"/);
...
test('editorial preset reaches every visual surface and all five typed renderers', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /<html lang="zh-CN" data-theme="dark" data-preset="editorial">/, mode);
    assert.match(svgBlock(html), /data-preset="editorial"/, mode);
    assert.match(html, /content: "编辑版 \/ 现场笔记"/, mode);
