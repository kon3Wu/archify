...
import { applyTemplate, renderCards, esc, diagramTypeLabel } from './utils.mjs';
...
export function svgAccessibleText(meta, kind) {
  const description = meta.subtitle || `这是 Archify 生成的${diagramTypeLabel(kind)}。`;
...
export function focusNodeAttrs(id, label, metadata = {}) {
  ...
  const detail = [metadata.sublabel, metadata.context]
    .filter((value) => value !== undefined && value !== null && String(value).trim() !== '')
    .join('、');
  const aria = detail ? `聚焦 ${label}，${detail}` : `聚焦 ${label}`;
